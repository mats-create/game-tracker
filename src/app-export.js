...
// ─── SHARED SVG HELPERS (used by both colour + embroidery export) ─────────
  ...
  function exportPDF(){
    ...
    (function(){
      ...
      // Precompute pentagon inner vertices (in panel coords relative to ball centre)
      var pentInner=[];
      for(var pi=0;pi<5;pi++){
        var pa=pi*Math.PI*2/5-Math.PI/2;
        pentInner.push([cx2+Math.cos(pa)*INNER_R, cy2+Math.sin(pa)*INNER_R]);
      }
      // Pentagon outer vertices (on ball edge)
      var pentOuter=[];
      for(var po=0;po<5;po++){
        var poa=po*Math.PI*2/5-Math.PI/2;
        pentOuter.push([cx2+Math.cos(poa)*BR, cy2+Math.sin(poa)*BR]);
      }
      // Mid-spoke outer points (halfway between pentagon spoke directions, at 0.75*BR)
      var midOuter=[];
      for(var pm=0;pm<5;pm++){
        var pma=(pm*Math.PI*2/5-Math.PI/2)+Math.PI*2/10;
        midOuter.push([cx2+Math.cos(pma)*BR*0.75, cy2+Math.sin(pma)*BR*0.75]);
      }

      // Point-to-segment distance helper
      function ptSegDist(px2,py2,ax,ay,bx2,by2){
        var dx=bx2-ax,dy=by2-ay;
        var lenSq=dx*dx+dy*dy;
        if(lenSq===0)return Math.hypot(px2-ax,py2-ay);
        var t=((px2-ax)*dx+(py2-ay)*dy)/lenSq;
        t=Math.max(0,Math.min(1,t));
        return Math.hypot(px2-(ax+t*dx),py2-(ay+t*dy));
      }

      // Point-in-polygon (ray casting) for pentagon
      function inPolygon(tx,ty,verts){
        var inside=false;
        for(var vi=0,vj=verts.length-1;vi<verts.length;vj=vi++){
          var xi=verts[vi][0],yi=verts[vi][1],xj=verts[vj][0],yj=verts[vj][1];
          if(((yi>ty)!=(yj>ty))&&(tx<(xj-xi)*(ty-yi)/(yj-yi)+xi))inside=!inside;
        }
        return inside;
      }

      var SPOKE_THRESH=CELL*0.72; // cells within this distance of a spoke line get spoke colour

      // Classify a cell centre and return fill colour
      function classifyCell(ccx,ccy){
        var dx=ccx-cx2,dy=ccy-cy2;
        var dist=Math.sqrt(dx*dx+dy*dy);

        // Outside ball entirely — linen background
        if(dist>BR+CELL*0.3)return '#F0E6D3';

        // Outer ring band
        if(dist>BR-RING_W){
          return isGhost?'#AAAAAA':'#333333';
        }

        if(isGhost){
          // Ghost ball — grey fill, grey spokes, no pentagon
          // Check spokes: inner vertex to outer vertex
          var onSpoke=false;
          for(var si=0;si<5;si++){
            if(ptSegDist(ccx,ccy,pentInner[si][0],pentInner[si][1],pentOuter[si][0],pentOuter[si][1])<SPOKE_THRESH){onSpoke=true;break;}
            if(ptSegDist(ccx,ccy,pentInner[si][0],pentInner[si][1],midOuter[si][0],midOuter[si][1])<SPOKE_THRESH){onSpoke=true;break;}
          }
          return onSpoke?'#AAAAAA':'#CCCCCC';
        }

        // Solid ball — check pentagon fill first
        if(inPolygon(ccx,ccy,pentInner))return '#1a1a1a';

        // Check spokes: inner vertex to outer vertex + inner to mid-outer
        for(var sk=0;sk<5;sk++){
          if(ptSegDist(ccx,ccy,pentInner[sk][0],pentInner[sk][1],pentOuter[sk][0],pentOuter[sk][1])<SPOKE_THRESH)return '#333333';
          if(ptSegDist(ccx,ccy,pentInner[sk][0],pentInner[sk][1],midOuter[sk][0],midOuter[sk][1])<SPOKE_THRESH)return '#333333';
        }

        // Inside ball, no other feature — white fill
        return '#ffffff';
      }

      var insetLines=[];

      // Linen background panel + pitch-green border
      insetLines.push('<rect x="'+px+'" y="'+py+'" width="'+PNL_W+'" height="'+PNL_H+'" fill="#F0E6D3" rx="3"/>');

      // Aida grid cells
      for(var row=0;row<ROWS;row++){
        for(var col=0;col<COLS;col++){
          var ccx=col*CELL+CELL/2;  // cell centre in panel-local coords
          var ccy=row*CELL+CELL/2;
          var fill=classifyCell(ccx,ccy);
          // Only draw non-linen cells explicitly; linen is the panel background
          if(fill!=='#F0E6D3'){
            insetLines.push('<rect x="'+(px+col*CELL).toFixed(1)+'" y="'+(py+row*CELL).toFixed(1)+'" width="'+CELL+'" height="'+CELL+'" fill="'+fill+'"/>');
          }
        }
      }

      // Grid lines — drawn over fills so the weave texture is visible
      var gl='stroke="#4A6741" stroke-width="0.4" opacity="0.35"';
      for(var gc=0;gc<=COLS;gc++){
        insetLines.push('<line x1="'+(px+gc*CELL)+'" y1="'+py+'" x2="'+(px+gc*CELL)+'" y2="'+(py+PNL_H)+'" '+gl+'/>');
      }
      for(var gr=0;gr<=ROWS;gr++){
        insetLines.push('<line x1="'+px+'" y1="'+(py+gr*CELL)+'" x2="'+(px+PNL_W)+'" y2="'+(py+gr*CELL)+'" '+gl+'/>');
      }

      // Panel border
      insetLines.push('<rect x="'+px+'" y="'+py+'" width="'+PNL_W+'" height="'+PNL_H+'" fill="none" stroke="#4A6741" stroke-width="1.5" rx="3"/>');

      // Label below panel
      var labelY=py+PNL_H+10;
      insetLines.push('<text x="'+(px+PNL_W/2)+'" y="'+labelY+'" text-anchor="middle" font-family="Inter,Helvetica,sans-serif" font-size="8" fill="#666666">14-ct aida \u00B7 reference only</text>');

      insetLines.forEach(function(l){svgLines.push(l);});
    })();
