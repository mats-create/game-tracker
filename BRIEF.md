...
The Aida Weave printout is generated by `exportPatternPDF()` in `app-export.js`.

### Pitch area
- Background: white (`#ffffff`)
- Stripe fills: faint greens (`#e8f0e8` / `#dceadc`) replacing the dark canvas greens
- Pitch lines: Pitch Green (`#4A6741`) — applied via regex substitution on `pitchSVGLines(false)` output
- All objects rendered: arrows, symbols, step markers, players, balls, legends
- Ghost players: rendered with `fill="none"`, dashed stroke in team colour — rendered separately **before** `playerSVGLines` (which receives only non-ghost players to avoid overpainting)
- Ghost balls: rendered with `fill="none"`, `stroke="#888888"`, dashed — rendered separately **before** `ballSVGLines` (which receives only non-ghost balls)
- Watermark: `moment.heading — moment.event` (Mid Grey, Inter, bottom-right inside pitch, omitted if both fields empty)

### Footer (below pitch, via jsPDF — not SVG)
The footer is drawn entirely by jsPDF after the pitch image is placed. This is the only reliable approach — SVG-embedded footers are subject to browser clip/render bugs.
- Separator line: light grey, just below the pitch image
- Left: "Nutmeg**&**Needle" wordmark (helvetica bold 7pt; `&` in Coral `#CC3300`, rest in `#1A1A1A`)
- Right: `nutmegneedle.com · [current year]` (helvetica normal 7pt, Mid Grey)

### Aspect ratio / circles
The SVG is built at the pitch's native pixel dimensions (`b.w × b.h`). The PDF placement dimensions are derived from this aspect ratio, constrained to fit within page margins minus the footer strip. This ensures circles remain circular in the output.
...
**Last completed:** Aida Weave printout improvements — Pitch Green lines, branding footer, watermark, ghost player/ball visibility, circle aspect ratio fix
...
| 2026-05-14 | Aida printout: pitch lines | Pitch lines changed to Pitch Green (#4A6741) via regex on pitchSVGLines output |
| 2026-05-14 | Aida printout: footer | Branding footer (line, logo, url, year) added below pitch via jsPDF — not SVG — for reliability |
| 2026-05-14 | Aida printout: watermark | moment.heading + moment.event concatenated, placed bottom-right inside pitch area |
| 2026-05-14 | Aida printout: ghost objects | Ghost players and balls now visible — rendered separately with print-safe colours before shared SVG helpers (which previously overwrote them with invisible rgba fills) |
| 2026-05-14 | Aida printout: circle fix | Aspect ratio now preserved by deriving pitchDH_PT from pitchDW_PT via SVG native ratio — prevents ovals |
